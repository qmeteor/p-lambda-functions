...
function saveResizedUrlLocation(userId, projectId, srcKey) {
    const params = {
        TableName: TABLE,
        Key: {
            imageId: '5a58e17041f18e378ed2b8b9/SK4WDBFKF/photo-48.jpg'
        },
        UpdateExpression: 'set #resizedUrl = :resizedUrl',
        ExpressionAttributeNames: { '#resizedUrl': 'resizedUrl' },
...
module.exports.upload = (event, context, callback) => {
    ...
    const srcKey = event.imageProperties.originFileObj.name;

    const userId = event.userId;
    const projectId = event.projectId;
    const imageProperties = event.imageProperties;
    ...
    const params = {
      ...
      Item: {
          userId: userId,
          projectImageId: projectId + '/' + srcKey,
          projectId,
          ...
          originalUrl: 'no location',
          resizedUrl: 'no location',
          imageTags: 'none'
      }
    };
...
module.exports.convert = (event, context, callback) => {
    ...
    const dstKey = replaceExt('resized-' + srcKey, '.jpg');
    ...
    // Validate image type
    // Allowed image types
    const allowed = ['jpg', 'png', 'CR2', 'tif', 'gif', 'NEF', '3FR'];
    ...
    const tempLocalFilename = `/tmp/${path.parse(srcKey).base}`;

    const file = fs.createWriteStream(tempLocalFilename);
    s3.getObject({
        Bucket: srcBucket,
        Key: srcKey
    }, function (err, data) {
        if (err) {
            ...
            fs.writeFile(tempLocalFilename, data.Body, function (err) {
                if (err) {
                    console.log('error while writing to tmp: ' + err);
                }
                else {
                    console.log('go to object in tmp');

                    if (fileExists(tempLocalFilename)) {
                        console.log('resizing to thumbnail and normalizing format to JPG');

                        const readStream = fs.createReadStream(tempLocalFilename);
                        gm(readStream)
                            .identify(function(err, data) {
                                if(err) {
                                    console.log('Error while identifying image properties');
                                } else {
                                                ...
                                                s3.putObject({
                                                    ...
                                                    if (err) {
                                                        ...
                                                        const params = {
                                                            ...
                                                            Key: {
                                                                userId: event.userId,
                                                                projectImageId: event.projectId + '/' + event.originalName
                                                            },
                                                            UpdateExpression: 'set resizedUrl = :resizedUrl',
                                                            ExpressionAttributeValues:{
                                                                ':resizedUrl':'resized-' + event.userId + '/' + event.projectId + '/' + event.originalName
                                                            },
                                                            ReturnValues:'UPDATED_NEW'
                                                        };
...
module.exports.get = (event, context, callback) => {
    ...
    console.log(`Querying for all images that belong to userId: ${userId} and projectId: ${projectId}.`);

    const params = {
        TableName : TABLE,
        ProjectionExpression:"userId, projectImageId, projectId, #imageDetails, #thumbnail",
        KeyConditionExpression: "userId = :id and begins_with (projectImageId, :pid)",
    ...
    docClient.query(params, function(err, data) {
        if (err) {
            ...
            let fileList = data.Items.map(function(item) {
...
module.exports.delete = (event, context, callback) => {
    ...
    console.log(`Delete single image that belongs to userId: ${userId} and projectId: ${projectId}.`);

    const params = {
        TableName: TABLE,
        Key:{
            "userId": userId,
            "projectImageId": projectId + '/' + fileName
        }
    };

    console.log("Attempting an unconditional delete...");
...
CUSTOM AUTHORIZER FUNCTIONS FROM BLUEPRINTS
